...
for k=2:length(et)-1
       for l=2:length(et)-1
           etanew(l,k)=eta(l,k)+exp((-(k-99)^2/(2*15^2))-(l-99)^2/(2*15^2)); %gør en gausisk
       end
end
f=5e-4;
alfa=dt*f;
...
utot(k,l)=(u(k,l)-beta*us(k,l)+alfa*vs(k,l))/(1+beta);
...
vtot(l,k)=(v(l,k)-beta*vs(l,k)-alfa*us(l,k))/(1+beta);
